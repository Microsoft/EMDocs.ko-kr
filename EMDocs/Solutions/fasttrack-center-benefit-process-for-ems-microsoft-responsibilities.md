---
title: Microsoft의 책임
description: 고객이 EMS용 FastTrack 센터 혜택을 사용할 때의 Microsoft 책임
keywords: ''
author: andredm7
ms.author: andredm
manager: ''
...
-   단일 포리스트 환경의 경우:
    ...
    -   Microsoft Intune(Azure Active Directory)과 Azure Active Directory Connect 도구의 암호 동기화(암호 해시) 구성. (필요한 경우에만 안내).
...
-   단일 및 다중 포리스트 환경의 경우:
    ...
    - 통과 인증 범위에 대한 자세한 내용은 [Azure Active Directory 통과 인증: 현재 제한 사항](https://docs.microsoft.com/azure/active-directory/connect/active-directory-aadconnect-pass-through-authentication-current-limitations)을 참조하세요.
...
-   관리 요구 사항에 따라 다음과 같이 모바일 장치 관리(MDM) 기관 구성.
    ...
    -   원격 지원, 모바일 위협 방지 파트너 및 Telecom Expense 파트너(Intune 구독에 포함되지 않은 타사 제품 라이선스)를 위해 Intune과 Team Viewer를 통합.
